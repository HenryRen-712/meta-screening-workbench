...
- 最终分析纳入确认：全文候选纳入研究由筛选者 A/B 再次独立盲法确认，冲突由裁决者处理；只有最终确认纳入的 studies 进入 Meta 数据提取。
- 数据提取：最终纳入文献可再次批量上传 PDF，按 DOI、PMID、完整题名自动匹配，并一键生成带原文证据、页码和 PDF SHA-256 的 Meta 数据提取草稿；表格可进入全屏工作区，并在 50%–130% 之间调整显示比例。
...
- 不做云同步和多人在线协作。
- 当前 A/B 独立确认依靠本地角色切换和界面盲法，尚无账号级权限隔离；两名筛选者应分别使用自己的角色操作。
